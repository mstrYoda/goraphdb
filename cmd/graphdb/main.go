...
func main() {
	...
	// Batch add edges
	edgeBatch := make([]graphdb.Edge, batchSize-1)
	for i := 0; i < batchSize-1; i++ {
		edgeBatch[i] = graphdb.Edge{From: ids[i], To: ids[i+1], Label: "next"}
	}
	...
	fmt.Printf("  Inserted %d edges in %v (%.0f edges/sec)\n",
		batchSize-1, time.Since(start),
		float64(batchSize-1)/time.Since(start).Seconds())
